...
class FinancialStatement(Base, TimestampMixin):
    ...
    statement_kind: Mapped[str | None] = mapped_column(String(80), nullable=True)
    gross_earnings: Mapped[Decimal | None] = mapped_column(Numeric(24, 4), nullable=True)
    interest_income: Mapped[Decimal | None] = mapped_column(Numeric(24, 4), nullable=True)
    net_interest_income: Mapped[Decimal | None] = mapped_column(Numeric(24, 4), nullable=True)
    customer_deposits: Mapped[Decimal | None] = mapped_column(Numeric(24, 4), nullable=True)
    loans_and_advances: Mapped[Decimal | None] = mapped_column(Numeric(24, 4), nullable=True)
    borrowings_total: Mapped[Decimal | None] = mapped_column(Numeric(24, 4), nullable=True)
    interest_expense: Mapped[Decimal | None] = mapped_column(Numeric(24, 4), nullable=True)
    npl_ratio: Mapped[Decimal | None] = mapped_column(Numeric(18, 4), nullable=True)
    capital_adequacy_ratio: Mapped[Decimal | None] = mapped_column(Numeric(18, 4), nullable=True)
    loan_to_deposit_ratio: Mapped[Decimal | None] = mapped_column(Numeric(18, 4), nullable=True)
    ...
    auditor_opinion: Mapped[str | None] = mapped_column(Text, nullable=True)
    major_risks: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)
    corporate_actions: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)
    source_document_id: Mapped[int | None] = mapped_column(ForeignKey("source_documents.id"))
    reviewed: Mapped[bool] = mapped_column(default=False)
